...
const CONFIG = {
  ...
  TIMER: {
    ...
    MAX_PARTICLES: 2000 // max particles when dissolving the timer
  },
...
const ArtExperience = () => {
  ...
  useEffect(() => {
    ...
    // Helper: toggle postprocessing for clean video playback
    const setPostProcessingEnabled = (enabled) => {
      if (threeRef.bloomPass) threeRef.bloomPass.enabled = enabled;
      if (threeRef.filmPass) threeRef.filmPass.enabled = enabled;
    };
    ...
    // --- Events
    // NOTE: define references so we can remove listeners later
    ...
    function onSelectionKeyPress(e) {
      ...
      if (e.key === 'ArrowLeft') {
        ...
        // when user explicitly selects, dissolve timer proportionally to remaining time
        const currentProgress = getTimerProgress();
        // if progress <= 0 then treat as timeout (clean removal)
        const dissolveTimer = currentProgress > 0;
        playSelectedVideo({ dissolveTimer, timerProgress: currentProgress });
      }
    }

    // Dissolve a generic mesh into particles (original)
    const dissolveObject = (object, particleCount = 5000) => {
      if (!object) return;
      try {
        // some objects might be non-buffer; try-catch to avoid crashes
        ...
        // fallback: just remove object if sampling fails
    ...
    // Dissolve the timer bar proportionally to remaining progress (0..1)
    // more remaining progress => more particles
    const dissolveTimerBar = (remainingProgress) => {
      if (!threeRef.timerBar) return;
      // clamp
      const p = THREE.MathUtils.clamp(remainingProgress, 0, 1);
      const count = Math.max(4, Math.floor(CONFIG.TIMER.MAX_PARTICLES * p));
      dissolveObject(threeRef.timerBar, count);
      threeRef.timerBar = null;
    };

    // Remove timer bar quietly (no particles)
    const removeTimerBarQuietly = () => {
      if (!threeRef.timerBar) return;
      if (threeRef.timerBar.parent) threeRef.timerBar.parent.remove(threeRef.timerBar);
      if (threeRef.timerBar.geometry) threeRef.timerBar.geometry.dispose();
      if (threeRef.timerBar.material) threeRef.timerBar.material.dispose();
      threeRef.timerBar = null;
    };

    // helper to get current remaining progress (remaining/duration)
    const getTimerProgress = () => {
      if (!threeRef.timer.active) return 0;
      ...
      return remaining / CONFIG.TIMER.DURATION;
    };
    ...
    // playSelectedVideo can accept options:
    // { dissolveTimer: boolean, timerProgress: number }
    const playSelectedVideo = (opts = { dissolveTimer: true, timerProgress: 0 }) => {
      if (threeRef.state !== 'selection') return;
      threeRef.state = 'playing';
      threeRef.timer.active = false;

      // Turn off bloom + film while playing so the video is clean
      ...
      // dissolve unselected panel as before
      if (unselected) {
        dissolveObject(unselected);
      }

      // If user selected early and dissolveTimer option is true -> dissolve timer proportionally
      if (opts.dissolveTimer && threeRef.timerBar) {
        const progress = typeof opts.timerProgress === 'number' ? opts.timerProgress : getTimerProgress();
        // Only dissolve if there's some remaining progress
        if (progress > 0) {
          dissolveTimerBar(progress);
        } else {
          // fallback: quiet remove
          removeTimerBarQuietly();
        }
      } else {
        // Clean timeout or explicit "no dissolve" request -> quietly remove
        removeTimerBarQuietly();
      }
    ...
    const onResize = () => {
      ...
      threeRef.renderer.setSize(mount.clientWidth, mount.clientHeight);
      if (threeRef.composer) threeRef.composer.setSize(mount.clientWidth, mount.clientHeight);
    };
    ...
    const animate = () => {
      ...
      for (let idx = threeRef.particleSystems.length - 1; idx >= 0; idx--) {
        const ps = threeRef.particleSystems[idx];
      ...
      if (threeRef.state === 'selection') {
        ...
        if (threeRef.timer.active && threeRef.timerBar) {
          ...
          // scale.x is used to represent remaining width; ensure geometry width used in position calculation
          threeRef.timerBar.scale.x = progress;
          // reposition the timer so it shrinks from right-to-left visually
          const originalWidth = threeRef.timerBar.geometry.parameters.width;
          threeRef.timerBar.position.x = -(1 - progress) * (originalWidth / 2);
          ...
          if (progress <= 0) {
            // Clean Timeout: remove timer quietly, do not spawn particles from timer
            // and proceed to play the currently selected video but with timer removed cleanly
            window.removeEventListener('keydown', onSelectionKeyPress);
            playSelectedVideo({ dissolveTimer: false, timerProgress: 0 });
          }
        }
      }
    ...
    return () => {
      ...
      // dispose scene resources conservatively
      threeRef.particleSystems.forEach(ps => {
        if (ps.parent) ps.parent.remove(ps);
